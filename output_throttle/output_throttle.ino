...
// Passes the RC servo us to serial output. I used it to get the min and max us the include controller outputs from the trigger.
...
void loop()
{
  ...
  if ((!rc_throttle_pin_state) && last_rc_throttle_pin_state) // falling edge
  {
    unsigned long temp = microseconds - last_rc_throttle_microseconds;
          Serial.println(temp); // does this add too much time to throw everything off??
  ...
  //        Serial.println(rc_throttle_servo_pulse_length); // does this add too much time to throw everything off??
...
//          Serial.println(rc_throttle_servo_pulse_length); // does this add too much time to throw everything off??
  ...
  if (rc_steering_servo_pulse_length)
  {
    ...
    throttle_servo.writeMicroseconds(constrain(rc_throttle_servo_pulse_length, 600, 2200));
